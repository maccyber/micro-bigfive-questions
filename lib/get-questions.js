...
module.exports = opts => {
  ...
  const file = isFile(path) ? require(path) : false
  if (!file) {
  ...
  res.next = res.page === res.totalPages ? false : `${opts.url}/?page=${opts.page + 1}&lang=${opts.langCode}`
  const reqQuestions = file.slice(res.from, res.to)
  res.questions = reqQuestions.map(q => { return { id: `Q${file.indexOf(q)}`, num: file.indexOf(q), text: q } })
  return res
}
