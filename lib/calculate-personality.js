'use strict'

const getPrefix = input => input === 'plus' ? '+' : '-'
const summarize = (a, b) => a + b
const objectize = (a, b) => {
  a[b.id] = b
  return a
}

module.exports = data => {
  if (!data || !data.template || !data.answers) {
    throw new Error('Missing required input')
  }

  const base = data.template.baseScore
  const questions = data.template.questions.reduce(objectize, {})
  const answers = data.answers
  let score = {
    E: [base['E']],
    A: [base['A']],
    C: [base['C']],
    N: [base['N']],
    O: [base['O']]
  }

  if (answers.length !== Object.keys(questions).length) {
    throw new Error('Mismatch of questions and  answers length')
  }

  answers.forEach(answer => {
    const template = questions[answer.id]
    const data = parseInt(`${getPrefix(template.type)}${answer.score}`, 10)
    score[template.personality].push(data)
  })

  score['E'] = score['E'].reduce(summarize, 0)
  score['A'] = score['A'].reduce(summarize, 0)
  score['C'] = score['C'].reduce(summarize, 0)
  score['N'] = score['N'].reduce(summarize, 0)
  score['O'] = score['O'].reduce(summarize, 0)

  return score
}
